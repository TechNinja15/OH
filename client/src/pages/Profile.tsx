...
export const Profile: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 overflow-y-auto custom-scrollbar pb-32 md:pb-6">
      <h2 className="text-3xl font-black mb-6 md:mb-8 flex justify-between items-center sticky top-0 bg-black/80 backdrop-blur-md py-2 z-10">
        <span>My Profile</span>
        {!isEditingProfile && (
          <button onClick={startEditing} className="text-xs md:text-sm font-bold text-neon border border-neon px-3 py-1.5 rounded-full hover:bg-neon hover:text-white transition-all flex items-center gap-2">
            <Edit2 className="w-3 h-3" /> <span className="hidden md:inline">Edit Profile</span><span className="md:hidden">Edit</span>
          </button>
        )}
      </h2>
      
      {isEditingProfile ? (
        <div className="bg-gray-900/50 border border-gray-800 p-4 md:p-6 rounded-3xl mb-6 animate-fade-in">
        ...
        // Display Mode
        <div className="bg-gray-900/50 border border-gray-800 p-5 md:p-8 rounded-3xl relative overflow-hidden mb-6 shadow-lg">
          ...
          <div className="relative z-10">
              ...
              <div className="bg-black/40 rounded-2xl p-4 mb-6 backdrop-blur-sm border border-gray-800/50">
                 ...
                 <div className="flex items-start justify-center md:justify-start gap-2 text-gray-400 border-t border-gray-700/50 pt-3">
                    <GraduationCap className="w-4 h-4 text-neon mt-0.5 flex-shrink-0" />
                    <span className="text-xs font-medium leading-relaxed">{currentUser?.university}</span>
                 </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        <NeonButton variant="danger" className="w-full justify-start mt-4 h-12" onClick={logout}>
